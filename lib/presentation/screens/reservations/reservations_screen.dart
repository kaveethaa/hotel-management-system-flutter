            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) => Center(
                child: ChoiceChip(
                  ...
                  label: Text(statuses[i]),

                  selected: _filter == statuses[i],
              ...
              data: (list) {
                ...
                return RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final r = f[i];
                      final guest = g.firstWhere(
                        (x) => x.id == r.guestId,
                        orElse: () => g.isNotEmpty ? g.first : throw '',
                      );
                      final room = rms.firstWhere(
                        (x) => x.id == r.roomId,
                        orElse: () => rms.first,
                      );
                      ...
                      return Card(
                        child: ListTile(
                          ...
                          subtitle: Text(
                            '${df.format(ci)} → ${df.format(co)} ${NumberFormat.currency(symbol: '\$').format(r.total)}',
                          ),
                          trailing: Chip(
                            label: Text(
                              r.status,
                          ...
                          onTap: () => context
                              .push('/reservations/form?id=${r.id}')
                              .then((_) {
                                ref.invalidate(reservationsProvider);
                                ref.invalidate(roomsProvider);
                              }),
