...
class _ReservationFormScreenState extends ConsumerState<ReservationFormScreen> {
  ...
  Future<void> _load() async {
    ...
    final r = await db.query(
      'reservations',
      where: 'id=?',
      whereArgs: [widget.reservationId],
      limit: 1,
    );
    if (r.isEmpty || !mounted) return;
    final res = Reservation.fromMap(r.first);
    setState(() {
      _guestId = res.guestId;
      _roomId = res.roomId;
      _ci = DateTime.parse(res.checkIn);
      _co = DateTime.parse(res.checkOut);
      _status = res.status;
    });
  }
  ...
  Future<void> _pickDate(bool isCheckIn) async {
    ...
    setState(() {
      if (isCheckIn) {
        _ci = d;
        if (!_co.isAfter(d)) _co = d.add(const Duration(days: 1));
      } else {
        _co = d;
      }
    });
  ...
  Future<void> _save() async {
    ...
    final avail = await isRoomAvailable(
      _roomId!,
      _ci,
      _co,
      excludeId: widget.reservationId,
    );
    if (!avail) {
      _snack('Room is not available for selected dates');
      return;
    }
    ...
    final res = Reservation(
      ...
      guestId: _guestId!,
      roomId: _roomId!,
      checkIn: _ci.toIso8601String(),
      checkOut: _co.toIso8601String(),
      status: _status,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(edit ? 'Edit Reservation' : 'New Reservation'),
        actions: [
          if (edit)
            IconButton(
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: _save,
                child: Text(edit ? 'Update' : 'Create Reservation'),
              ),
            ),
          ],
        ),
